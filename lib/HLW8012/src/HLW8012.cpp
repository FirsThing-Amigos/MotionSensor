...
void HLW8012::begin(
    ...
    _calculateDefaultMultipliers(1,1,1);
...
void HLW8012::setResistors(double current, double voltage_upstream, double voltage_downstream,float current_callibration, float voltage_callibration, float power_callibration) {
    if (voltage_downstream > 0) {
        _current_resistor = current;
        _voltage_resistor = (voltage_upstream + voltage_downstream) / voltage_downstream;
        _calculateDefaultMultipliers(current_callibration,voltage_callibration,power_callibration);
    }
}
...
void HLW8012::_calculateDefaultMultipliers(float current_callibration, float voltage_callibration, float power_callibration) {
    _current_multiplier = ( 1000000.0 * 512 * V_REF / _current_resistor / 24.0 / F_OSC )*current_callibration;
    _voltage_multiplier = ( 1000000.0 * 512 * V_REF * _voltage_resistor / 2.0 / F_OSC )*voltage_callibration;
    _power_multiplier = ( 1000000.0 * 128 * V_REF * V_REF * _voltage_resistor / _current_resistor / 48.0 / F_OSC )*power_callibration;
}
